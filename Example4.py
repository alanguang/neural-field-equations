...
def pde(x, V):
    ...
    def integrand(y1, y2, x1, x2, t):
        ...
        tau = (tf.square(x1 - y1) + tf.square(x2 - y2))/v
        t_tau = t - tau
        V_y = tf.exp(-tf.nn.relu(t_tau)) * tf.exp(-tf.square(y1) - tf.square(y2))  # 使用 ReLU 确保非负性
        return K_val * V_y
    ...
    # return I_val - V_val + integral - V_t
    return I_val - V_val + integral - V_t + noise
...
for t in t_values:
    ...
    ax1.set_title(f"Example 4: t={t} predicted solutions")
    ax1.set_xlabel('X')
    ax1.set_ylabel('Y')
    ax1.set_zlabel('Z')
    plt.tight_layout()
    ...
    ax2.set_title(f"Example 4: t={t} true solutions")
    ax2.set_xlabel('X')
    ax2.set_ylabel('Y')
    ax2.set_zlabel('Z')
    plt.tight_layout()
    ...
    ax3.set_title(f"Example 4: t={t} Error plot between true and predicted solutions")
    ax3.set_xlabel('X')
    ax3.set_ylabel('Y')
